...
class SLAM:
    ...
    def run(self):
        ...
        while self.main_thr.is_alive():
            ...
            if self.odom and self.lidar:
                self.process_SLAM()
                time.sleep(0.3)
            if not self.optimizing and len(self.node_queue) > 0:
                print("opt")
    ...
    def optimize(self):
        ...
        last_reg_odom = self.pose_graph[-1, "pos"]
        print("lro", self.pose_graph[-1, "pos"])
        ...
        for i in range(5):
            self.SLAM_add_edges()
            self.Gauss_Newton()
        print(self.robot.increment_by_wheels)
        corr_odom = last_reg_odom - np.array(self.robot.increment_by_wheels)
        #corr_odom *= -1
        print(corr_odom)
        print("Nlro", self.pose_graph[-1, "pos"])
        self.robot.calculated_pos = [*pos_vector_from_homogen_matrix(homogen_matrix_from_pos(last_reg_odom) @ homogen_matrix_from_pos(corr_odom))]
        self.optimizing = False
    ...
    def process_SLAM(self):
        ...
        if point_cloud.peak_coords[0]:
            ...
            if not nn:
                self.node_queue.append([self.odom, point_cloud, self.path_length])
            ...
            if icp_out and icp_out[-1] < 0.03 and err[0] ** 2 + err[1] ** 2 < 1 and abs(err[2]) < 0.7:
                if self.path_length - self.last_path > 0.3:
                    self.node_queue.append([self.odom, point_cloud, self.path_length])
                    self.last_path = self.path_length
                return
        if self.path_length - self.last_path > 0.8:
            self.node_queue.append([self.odom, point_cloud, self.path_length])
            self.last_path = self.path_length
