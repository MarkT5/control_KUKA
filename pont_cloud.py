    ...
    def __init__(self, data):
        ...
        for i in range(len(self.gaps) - 1, 0, -1):
            ...
            for i in range(1, len(dp_out[0]) - 1):
                self.peaks.append(
                    [[dp_out[0][i - 1], dp_out[0][i], dp_out[0][i + 1]], [dp_out[1][i - 1], dp_out[1][i]]])
    ...
    def split_objects(self):
        ...
        for i in range(1, len(lidar)):
            ...
            if abs(lidar[i - 1] - lidar[i]) < 0.1:
                object_sizes[-1][1] += 1
                lid_ang = i * math.radians(240) / len(self.lidar) - math.radians(30)
                lid_dist = self.lidar[i]
                if lid_dist > 5.5:
                    continue
        ...
        self.gaps = gaps
        self.xy_local_form = np.array(xy_local_form)
        self.object_sizes = sorted(object_sizes, key=lambda x: x[1], reverse=True)

        return np.array(xy_local_form)

    def xy_local_to_global(self):
        out_points = np.array(self.xy_local_form)
        ...
        for a in range(iters):
            for i in range(1, len(point_list)):
                ...
                if point_list[i] - point_list[i - 1] > 5:
                    add_point = self.find_farthest(list(self.xy_local_form[point_list[i - 1]:point_list[i]]), 0.1)
                if not add_point:
